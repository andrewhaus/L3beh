...
void objLoaderToGLM(objl::Loader&, std::vector<glm::vec3>&,
...
void RenderSystem::loadModel(RenderComponent& rc){
    if (loadedVertexBuffers.find(rc.modelFileName) == loadedVertexBuffers.end()){
        ...
        std::string fileExt = GetFileExtension(rc.modelFileName);
        if ( fileExt.compare( "obj" ) == 0){
            objl::Loader loader;
            loader.LoadFile(("graphics/" + rc.modelFileName).c_str());
            objLoaderToGLM(loader, vertices, texCoords, normals);
        }else if ( fileExt.compare( "blend" ) == 0){
            // TODO: add loading Blend models directly
            // fbtBlend fp;
            // if (fp.parse(("graphics/" + rc.modelFileName).c_str())!=fbtFile::FS_OK) {
            //   std::cerr << "unable to find file for model.\n";
            //   return;
            // }
            // long cnt=-1;
          ...
          std::cerr << "the model you are trying to use is not compatible with this engine.\n";
          return;
        }
...
// load vertex, texture coordinates, and normals
void objLoaderToGLM(objl::Loader &loader, std::vector<glm::vec3>&vertices, std::vector<glm::vec2> &texCoords, std::vector<glm::vec3> &normals) {
    for (uint i = 0; i < loader.LoadedVertices.size(); i++) {
      ...
      glm::vec2 textCoords;
      textCoords.x = loader.LoadedVertices[i].TextureCoordinate.X;
      textCoords.y = loader.LoadedVertices[i].TextureCoordinate.Y;
